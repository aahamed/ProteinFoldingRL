...
class Pulling2DEnv(gym.Env):
    ...
    def __init__(self, seq, collision_penalty=-2, trap_penalty=0.5, pull_reward=0):
    ...
    def step( self, action ):
        ...
        if ACTION_TO_STR[ pull_dir ] == 'STOP':
            self.done = True
            reward = self._compute_reward(False, False, False)
            info = {
        ...
        reward = self._compute_reward(False, collision, True)
        info = {
            'chain_length' : len(self.chain),
            'seq_length'   : len(self.seq),
            'collisions'   : self.collisions,
    ...
    def _compute_reward(self, is_trapped, collision, pulled):
        ...
        pull_reward = self.pull_reward if pulled else 0

        # Compute reward at timestep, the state_reward is originally
        # negative (Gibbs), so we invert its sign.
        reward = - state_reward + collision_penalty + actual_trap_penalty + pull_reward
